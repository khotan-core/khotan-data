...
Wire is a self-contained fetch wrapper component for external APIs. It provides pluggable auth strategies, retry with exponential backoff, pagination helpers, rate limit handling, timeout support, and typed HTTP methods — all in a single editable file with zero runtime dependencies on khotan-data.
