...
Plug Client provides typed, validated HTTP endpoints on top of the Plug fetch wrapper. A contract defines the API shape (method, path, params, query, body, response schemas), and a library-provided adapter (`createPlugClient`) bridges that contract to the Plug's execution layer. The adapter lives in the `khotan-data/plug` subpath export — it is not scaffolded into the user's project.
